...
| `humanizer-zh` | none | pending | not found in specified sources | No real `Humanizer-zh` skill was found in the specified sources. Closest general writing substitute found: `email-draft-polish` in https://github.com/ComposioHQ/awesome-codex-skills, but it is email-specific and not Chinese-humanizer-specific, so it is not installed here. |
...
- `humanizer-zh`: pending. No matching real skill was found in the specified sources.
